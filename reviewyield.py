...
if st.button("查询数据"):
    # 构建查询条件
    query = """
    SELECT YYWW, SUM(quantity) as total_quantity, 
    ...
    # 添加PN/Module筛选条件
    if search_text:
        query += " AND (pn LIKE ? OR modulename LIKE ?)"
    ...
    query += " GROUP BY YYWW ORDER BY YYWW"
    ...
    if not df.empty:
        ...
        # 使用Streamlit原生图表
        st.subheader("不良率趋势")
        st.line_chart(df.set_index('YYWW')['defect_rate'])
        
        st.subheader("生产数量和不良数量")
        st.bar_chart(df.set_index('YYWW')[['total_quantity', 'total_defects']])
        
        # 新增：各PN详细数据查询
        detail_query = """
        SELECT pn, modulename, YYWW, 
        ...
        if search_text:
            detail_query += " AND (pn LIKE ? OR modulename LIKE ?)"
            detail_params.extend([search_term, search_term])
        elif selected_pn != "All":
            detail_query += " AND pn = ?"
            detail_params.append(selected_pn)
            
        detail_query += " GROUP BY pn, modulename, YYWW ORDER BY pn, YYWW"
        ...
        st.subheader("不良内容详情")
        defect_query = """
        SELECT pn, YYWW, defect_reason, improvement, SUM(defect_count) as defect_count
        FROM productiondata 
        WHERE YYWW >= ? AND YYWW <= ? AND defect_count > 0
        """
        defect_params = [start_yyww, end_yyww]
        
        if search_text:
            defect_query += " AND (pn LIKE ? OR modulename LIKE ?)"
            defect_params.extend([search_term, search_term])
        elif selected_pn != "All":
            defect_query += " AND pn = ?"
            defect_params.append(selected_pn)
            
        defect_query += " GROUP BY pn, YYWW, defect_reason, improvement ORDER BY pn, YYWW"
        
        defect_df = pd.read_sql(defect_query, conn, params=defect_params)
        
        if not defect_df.empty:
            for index, row in defect_df.iterrows():
                with st.expander(f"PN: {row['pn']} | 周: {row['YYWW']} | 不良数量: {row['defect_count']}"):
                    st.write(f"不良原因: {row['defect_reason']}")
                    st.write(f"改善措施: {row['improvement']}")
        else:
            st.write("未找到匹配的不良记录")
...
def init_production_db():
    ...
    if cursor.execute("SELECT COUNT(*) FROM productiondata").fetchone()[0] == 0:
        users = ['张三', '李四', '王五']
        defect_reasons = ['材料问题', '工艺问题', '设备故障', '操作失误', '设计缺陷']
        improvements = ['更换供应商', '调整参数', '设备维护', '加强培训', '设计优化']
        ...
        for i in range(8):
            ...
            for pn, module in PN_TO_MODULE.items():
                ...
                (pn, modulename, username, createdate, YYWW, quantity, defect_count, defect_reason, improvement, remark)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    pn,
                    module,
                    ...
                    quantity,
                    defect_count,
                    random.choice(defect_reasons) if defect_count > 0 else '',
                    random.choice(improvements) if defect_count > 0 else '',
                    ''
                ))
